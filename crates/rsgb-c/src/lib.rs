//! C source backend for rsgb programs.
//!
...
use rsgb::node::REDUCE_SIMD_MIN;
use rsgb::{BinOp, CmpOp, ExternBundle, ReduceOp, Tape, TapeVisitor, UnaryOp};
...
#define RSGB_EXP_LIMIT 80.0
#define RSGB_LN_FLOOR 1e-30

static double rsgb_exp(double x) {
    if (x > RSGB_EXP_LIMIT) return exp(RSGB_EXP_LIMIT) * (1.0 + (x - RSGB_EXP_LIMIT));
    return exp(x);
}
static double rsgb_ln(double x) {
    if (x > RSGB_LN_FLOOR) return log(x);
    return log(RSGB_LN_FLOOR);
}
static double rsgb_sqrt(double x) { return x > 0.0 ? sqrt(x) : 0.0; }
static double rsgb_sign(double x) { return x > 0.0 ? 1.0 : (x < 0.0 ? -1.0 : x); }
static double rsgb_powi(double a, int32_t b) {
...
static double rsgb_digamma(double x) {
    double result = 0.0;
...
static double rsgb_trigamma(double x) {
    double result = 0.0;
...
static double rsgb_rand_uniform(double key) {
    uint64_t z;
...
impl TapeVisitor for Emitter {
    ...
    fn powi(&mut self, dst: u32, a: u32, n: i32) {
        self.set(dst, &format!("rsgb_powi(work[{a}], {n})"));
    }
