...
mod tests {
    ...
    fn rebuild_drops_dead_nodes_and_keeps_values() {
        ...
        env.insert(crate::node::SymbolId(0), 0.7);
        env.insert(crate::node::SymbolId(1), -1.3);
        let a = eval_real(&g, &env, &[root]);
        let b = eval_real(&h, &env, &roots);
        assert_eq!(a[0].to_bits(), b[0].to_bits());
        assert_eq!(h.symbol_name(crate::node::SymbolId(1)), "y");
    }
    ...
    fn rebuild_carries_functions_and_calls() {
        ...
        let (h, roots) = rebuild(&g, &[root]);
        let mut env = HashMap::new();
        env.insert(crate::node::SymbolId(1), 3.0);
