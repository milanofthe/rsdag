...
//! whole op vocabulary.
//!
//! Every guarantee rsgb makes is a statement over arbitrary programs -- the
...
//!
//! ```
//! use rsgb::{synth::{Spec, build, inputs}, Graph, Tape, F64};
...
mod tests {
    ...
    fn every_in_crate_path_matches_the_arena() {
        // The paths rsgb itself offers, over a corpus that varies size and
    ...
    fn smooth_programs_differentiate_and_evaluate_like_the_arena() {
        for seed in 0..8u64 {
            ...
            let (roots, syms) = build(&mut g, &mut spec);
            let d = crate::differentiate(&mut g, roots[0], syms[0]);
            ...
            let want = crate::eval_real(&g, &env, &[roots[0], d]);
            assert!(
                out.iter()
                    .zip(&want)
